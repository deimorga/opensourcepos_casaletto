<?php

return [
    "already_open"               => "Another shift is still open. Close it before opening a new one.",
    "amount"                     => "Amount",
    "amount_number"              => "Amount must be a number",
    "amount_required"            => "Amount is a Required Field.",
    "cancel_cashups"             => "",
    "cancel_cashups_enter"       => "",
    "cannot_be_deleted"          => "Cashup cannot be deleted",
    "cannot_edit_closed"         => "A closed cashup cannot be modified",
    "cash_difference"            => "",
    "close_date"                 => "Close Date",
    "close_employee"             => "Closed By",
    "closed_amount_card"         => "Cards",
    "closed_amount_cash"         => "Closed Cash",
    "closed_amount_check"        => "Checks",
    "closed_amount_due"          => "Dues",
    "closed_amount_giftcard"     => "",
    "closed_amount_total"        => "Total",
    "closed_date"                => "Closed Date",
    "collection_add"             => "Record collection",
    "collection_amount"          => "Amount collected",
    "collection_collected_at"    => "Taken on",
    "collection_collected_by"    => "Taken by",
    "collection_collector_invalid" => "Only an administrator can be recorded as having taken the cash.",
    "collection_confirm_delete"  => "Remove this collection?",
    "collection_delete"          => "Remove this collection",
    "collection_deleted"         => "Collection removed",
    "collection_error"           => "The collection could not be saved",
    "collection_history"         => "Already recorded",
    "collection_invalid_date"    => "Enter a valid date and time for the collection.",
    "collection_none"            => "No cash has been collected from this shift.",
    "collection_note"            => "Note",
    "collection_saved"           => "Collection recorded",
    "collections"                => "Cash collections",
    "confirm_delete"             => "Are you sure you want to delete the selected Cashup?",
    "confirm_reopen"             => "Are you sure you want to reopen the selected Cashup(s)? This will allow their closing data to be modified again.",
    "confirm_restore"            => "Are you sure you want to restore selected Cashup(s)?",
    "confirm_submit"             => "",
    "date_number"                => "Date must be a number",
    "date_required"              => "Date is a required field",
    "delete"                     => "Allow delete",
    "delete_not_allowed"         => "You do not have permission to delete cashups",
    "description"                => "Description",
    "enable_expected"            => "",
    "error_adding_updating"      => "Error adding/updating Cashup",
    "giftcard"                   => "",
    "id"                         => "Id",
    "invalid_amount"             => "Enter a valid amount for {0} -- it cannot be left blank or unreadable.",
    "info"                       => "Cashups Info",
    "info_employee"              => "",
    "is_deleted"                 => "Deleted",
    "new"                        => "New Cashup",
    "no_cashups_to_display"      => "There are no Cashups to display",
    "none_selected"              => "You have not selected any Cashups",
    "note"                       => "Notes",
    "one_or_multiple"            => "Cashups(s)",
    "open_amount_cash"           => "Open Cash",
    "open_date"                  => "Open Date",
    "open_employee"              => "Opened By",
    "opened_date"                => "Opened Date",
    "reconciliation"             => "Drawer reconciliation",
    "reconciliation_collections" => "Cash collected",
    "reconciliation_counted"     => "Counted (declared)",
    "reconciliation_detail"      => "See the breakdown",
    "reconciliation_discrepancy" => "Difference",
    "reconciliation_expected"    => "Expected in drawer",
    "reconciliation_expenses"    => "Expenses paid from the drawer",
    "reconciliation_income"      => "Taken in this shift",
    "reconciliation_open"        => "Opening float",
    "reconciliation_unsealed"    => "This shift has no sales linked to it, so what it took in cannot be shown.",
    "reopen"                     => "Reopen",
    "reopen_admin_only"          => "You do not have permission to reopen cashups",
    "status"                     => "Status",
    "status_closed"              => "Closed",
    "status_open"                => "Open",
    "successful_adding"          => "Cashup add successful",
    "successful_deleted"         => "Cashup delete successful",
    "successful_reopen"          => "Cashup reopened successfully",
    "successful_updating"        => "Cashup update successful",
    "tab_shift"                  => "Shift",
    "total"                      => "Total",
    "transfer_amount_cash"       => "In/Out Cash",
    "transfer_amount_cash_minus" => "",
    "update"                     => "Update Cashup",
    "warning"                    => "",
];
