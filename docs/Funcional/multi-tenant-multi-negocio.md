...
Fase 0 (documentación), Fase 1 (cerrar huecos de sede en ventas, turnos, gastos, recepciones y mesas) y Fase 2 (actualización de MariaDB y PHP a versiones con soporte vigente) completas — sin impacto visible para el usuario final. **Las tres ya están desplegadas en staging y producción reales** (Fase 1 el 2026-07-30, verificado 100% de filas con `location_id`, sin errores). Fase 3 (schema donde vivirá el registro de negocios-cliente) y Fase 4 (el mecanismo que detecta a qué negocio pertenece cada visita según la URL, y aísla su conexión a datos) completas, solo en la rama de desarrollo — todavía no hay nada visible para ningún usuario, y probado a fondo que Casaletto sigue funcionando exactamente igual mientras no esté registrado como negocio-cliente. Detalle técnico en `docs/Tecnico/multi-tenant-arquitectura.md`.
