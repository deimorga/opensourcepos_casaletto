...
Fase 6 (la infraestructura de enrutamiento que permitirá que cada negocio tenga su propia dirección web tipo `negocio.pos-casaletto.micronuba.net`) completa **y ya desplegada en staging y producción reales** — sin ningún subdominio de negocio activo todavía (falta el registro DNS, que se crea recién cuando haya un negocio real que dar de alta), y sin ningún cambio visible para Casaletto, que sigue entrando exactamente por la misma dirección de siempre.
