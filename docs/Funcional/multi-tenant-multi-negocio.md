...
Fase 0 (documentación), Fase 1 (cerrar huecos de sede en ventas, turnos, gastos, recepciones y mesas) y Fase 2 (actualización de MariaDB y PHP a versiones con soporte vigente) completas — sin impacto visible para el usuario final. **Las tres ya están desplegadas en staging y producción reales** (Fase 1 el 2026-07-30, verificado 100% de filas con `location_id`, sin errores). Fase 3 (schema donde vivirá el registro de negocios-cliente) completa, solo en la rama de desarrollo — todavía no hay nada visible para ningún usuario, es la primera pieza interna de la arquitectura multi-tenant. Detalle técnico en `docs/Tecnico/multi-tenant-arquitectura.md`.
