...
- **Fase 6 (infraestructura Traefik wildcard + Docker Compose)**: completa.
  ...
  - **Riesgo de colisión real identificado y resuelto**: el wildcard de producción (`*.pos-casaletto.micronuba.net`) matchea literalmente el propio host de staging (`staging.pos-casaletto.micronuba.net`), porque "staging" es una cadena válida de un solo nivel — y ambos comparten el mismo Traefik. Se resuelve con dos capas: (1) `priority` explícito más alto en el router de staging (20) que en el de producción (10), para que Traefik desempate siempre a favor de staging si alguna vez coincide; (2) **`staging` (junto con `www`, `admin`, `platform`, `login`, `api`, `app`) queda reservado y nunca se debe asignar como slug real de tenant** — la herramienta de provisión de la Fase 7 debe validar esto explícitamente contra una lista de slugs prohibidos, no asumir que nunca pasará.
  - `certresolver` de ambos routers pasa de `le` a `le-dns`: no es estrictamente necesario para que HTTP-01 funcione por subdominio individual, pero evita agotar los rate-limits de Let's Encrypt a medida que se agreguen tenants y no depende de que el puerto 80 esté alcanzable por cada subdominio nuevo — mismo criterio ya usado por el otro proyecto en este Traefik para su propio escenario multi-subdominio.
  - `App::$allowedHostnameWildcards` configurado vía `ALLOWED_HOSTNAME_WILDCARDS` en ambos compose (`.pos-casaletto.micronuba.net` en prod, `.staging.pos-casaletto.micronuba.net` en staging) — reusa el mecanismo ya construido en la Fase 4.
  ...
  - **No se creó el registro DNS wildcard todavía** (`*.pos-casaletto.micronuba.net` / `*.staging.pos-casaletto.micronuba.net` → IP del VPS) — sin eso, ningún subdominio de tenant es alcanzable aún; el cambio de Traefik es inerte para tráfico real hasta que exista, y es intencional: no rompe nada mientras tanto. Crear esos registros DNS queda para cuando haya tenants reales que provisionar (Fase 7 en adelante).
  - **Desplegado a staging y producción reales (2026-07-31)**, a diferencia de las Fases 3-5. Validado en ambos: labels de Traefik aplicadas correctamente (`docker inspect`), `HTTP 200` en los dos dominios exactos de siempre (sin cambio de comportamiento), certificado existente (`le`, HTTP-01) sigue sirviéndose sin interrupción — Traefik no fuerza una re-emisión inmediata solo porque cambió el `certresolver` del router; `le-dns` recién se ejercita cuando llegue tráfico real a un subdominio nuevo, algo que hoy no puede pasar sin el DNS wildcard. Cero errores/warnings en los logs de Traefik, y confirmado que sitios hermanos en el mismo Traefik compartido (`staging.plagie.cloud`, `crm.micronuba.net`) siguen respondiendo igual.
